...
import type { OctalToBinaryStep } from "@/types/animation";
import { motion, AnimatePresence } from "framer-motion";

interface OctalToBinaryAnimationProps {
  step: OctalToBinaryStep | null;
}
...
export function OctalToBinaryAnimation({ step }: OctalToBinaryAnimationProps) {
  if (!step) return <div className="p-4 text-muted-foreground">Animation will appear here.</div>;

  const renderStep = () => {
    switch (step.type) {
      case "INITIAL_OCT_TO_BIN":
        return (
          <div className="space-y-3">
            <p>Converting: <span className="font-bold text-primary text-xl font-mono">{step.octalInput}</span> (Octal) to Binary</p>
            <p className="text-sm text-muted-foreground">{step.explanation}</p>
          </div>
        );
      case "PROCESS_OCT_DIGIT":
        return (
          <div className="space-y-3">
            <p>Processing octal digit {step.digitIndex + 1} of {step.allOctalDigits.length}:</p>
            <div className="font-mono text-2xl flex space-x-1 bg-muted p-3 rounded-md justify-center items-center flex-wrap">
              {step.allOctalDigits.map((digit, index) => (
                 <motion.span 
                    key={index} 
            ...
            <div className="text-center mt-3 p-3 bg-muted/50 rounded-md">
              <p className="text-lg">
                Digit <span className="font-mono font-bold text-primary">{step.octalDigit}</span> converts to 
                <span className="font-mono font-bold text-2xl text-green-400 ml-2 p-1 bg-green-400/10 rounded">{step.binaryEquivalent}</span> (3-bit Binary)
              </p>
            </div>
      ...
      case "FINAL_RESULT_OCT_TO_BIN":
        return (
          <div className="space-y-3">
            <p>Original Octal: <span className="font-bold text-primary font-mono">{step.originalOctal}</span></p>
